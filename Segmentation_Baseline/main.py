...
if __name__ == '__main__':
    ...
    # label_list = np.asarray(label_list)
    total_cases = len(data_list)
    test_dataset = StestDataset(data_list, label_list)
    ...
    if args.inference_model != None:
        ...
        test_loss, dice, iou, dice_std, iou_std = classifier.test(test_loader, args.inference_model)
        log_file = open(os.path.join(project_path, record_path, args.inference_model, "log.log"), "a")
        log_file.writelines(
            "Final !! Test Loss: {}, Test Dice: {}+={}, IOU: {}+={}\n"
            .format(test_loss, dice, dice_std, iou, iou_std))
        log_file.writelines(str(datetime.now())+"\n")
        log_file.close()
    else:
        test_loss, dice, iou, dice_std, iou_std = classifier.test(test_loader, args.inference_model)
